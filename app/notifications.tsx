...
// Mock notifications data (outside component to avoid recreation)
...
function getBackgroundColor(type: string, read: boolean) {
...
// Separate NotificationItem component with memoization
...
const NotificationItem = memo(({ notification }: NotificationItemProps) => {
  ...
  const backgroundColor = getBackgroundColor(notification.type, notification.read);
  ...
  return (
    ...
    >
      <View className="flex-row items-start gap-3">
        ...
        >
          <Text className="text-[16px]" style={{ color: iconColor }}>
            {icon}
          </Text>
        </View>
        <View className="flex-1">
          <View className="flex-row items-start justify-between mb-1">
            ...
            {!notification.read && (
              <View className="w-[8px] h-[8px] rounded-full bg-violet-400 ml-2 mt-1" />
            )}
          </View>
          <Text className="text-[13px] text-[#9ca3af] mb-2">
            {notification.message}
          </Text>
          <Text className="text-[11px] text-[#6b7280]">{notification.time}</Text>
        </View>
      </View>
    </Pressable>
  );
